...
export class SessionDescriptionHandler {
    ...
    setRemoteTrack(track) {
        ...
        if (remoteStream.getTrackById(track.id)) {
            ...
            this.logger.debug(`SessionDescriptionHandler.setRemoteTrack - adding remote ${track.kind} track`);
            if (track.label === "screen_label") {
                // 此视频流为屏幕共享
                // remoteScreenStream.getVideoTracks().forEach((track) => {
                //     track.stop();
                //     remoteScreenStream.removeTrack(track);
                //     SessionDescriptionHandler.dispatchRemoveTrackEvent(remoteScreenStream, track);
                // });
                if(remoteScreenStream.getVideoTracks().length>0)return
                remoteScreenStream.addTrack(track);
                SessionDescriptionHandler.dispatchAddTrackEvent(remoteScreenStream, track);
            }
            else {
